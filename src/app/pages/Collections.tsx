  ...
  return (
    <div ref={fu.ref} style={fu.s}>
      <Link to={`/collections/${c.slug}`} style={{ textDecoration: "none", color: "inherit", display: "block" }}>
        ...
        >
          <h3 style={{ fontFamily: DISPLAY, fontSize: 28, color: HEADING_INK, margin: "0 0 8px", letterSpacing: "-.5px" }}>{c.name}</h3>
          <p style={{ fontFamily: SANS, fontSize: 14.5, color: INK, opacity: .68, lineHeight: 1.6, margin: "0 0 18px", maxWidth: 320 }}>{c.subtitle}</p>
          <span style={{ fontFamily: DISPLAY, fontSize: 13, letterSpacing: ".5px", textTransform: "uppercase", color: INK, display: "inline-flex", alignItems: "center", gap: 6 }}>
  ...
  return (
    <section style={{ background: BG, paddingTop: 72, minHeight: "100vh" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "48px 24px 8px" }}>
        <div style={{ fontFamily: SCRIPT2, fontSize: 22, color: INK, opacity: .55, marginBottom: 6 }}>— curated —</div>
        <h1 style={{ fontFamily: DISPLAY, fontSize: "clamp(34px,5vw,64px)", color: HEADING_INK, margin: "0 0 12px", letterSpacing: "-1px", textTransform: "uppercase" }}>Collections</h1>
        <p style={{ fontFamily: SANS, fontSize: 16, color: INK, opacity: .6, margin: 0 }}>Shade edits curated for the moments that matter most.</p>
      </div>
