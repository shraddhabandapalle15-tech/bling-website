...
const fieldStyle: CSSProperties = {
  ...
  fontFamily: SANS, fontSize: TEXT_MD, color: INK, background: "#fff", outline: "none", boxSizing: "border-box",
};
    ...
    return (
      <section style={{ background: BG, paddingTop: 72, minHeight: "100vh", display: "flex", alignItems: "center" }}>
        <div style={{ maxWidth: 560, margin: "0 auto", padding: "24px", textAlign: "center" }}>
          ...
          <Link to="/shop" className="pbtn" style={{ display: "inline-block", background: YELLOW, color: INK, borderRadius: 100, padding: "16px 36px", fontFamily: DISPLAY, fontSize: TEXT_BASE, letterSpacing: "1px", textTransform: "uppercase", textDecoration: "none" }}>
            Continue Shopping
          </Link>
        </div>
      </section>
    ...
    return (
      <section style={{ background: BG, paddingTop: 72, minHeight: "100vh", display: "flex", alignItems: "center" }}>
        <div style={{ maxWidth: 480, margin: "0 auto", padding: "24px", textAlign: "center" }}>
          ...
          <p style={{ fontFamily: SANS, fontSize: TEXT_MD, color: INK, opacity: .6, margin: "0 0 28px" }}>Add a few shades before checking out.</p>
          <Link to="/shop" className="pbtn" style={{ display: "inline-block", background: YELLOW, color: INK, borderRadius: 100, padding: "16px 36px", fontFamily: DISPLAY, fontSize: TEXT_BASE, letterSpacing: "1px", textTransform: "uppercase", textDecoration: "none" }}>
            Shop Glitter
          </Link>
        </div>
    ...
    <section style={{ background: BG, paddingTop: 72, minHeight: "100vh" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "56px 24px 100px" }}>
        ...
        <form onSubmit={handleSubmit} className="shopLayout" style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 48, alignItems: "start" }}>
          <div>
            <h2 style={{ fontFamily: DISPLAY, fontSize: TEXT_XL, color: HEADING_INK, margin: "0 0 18px", letterSpacing: "-.2px" }}>Customer Details</h2>
            <div className="featGrid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
              <div>
                <label style={{ display: "block", fontFamily: SANS, fontSize: TEXT_SM, color: INK, opacity: .6, marginBottom: 7 }}>Full Name</label>
            ...
            <div style={{ maxWidth: 340, marginBottom: 40 }}>
              <label style={{ display: "block", fontFamily: SANS, fontSize: TEXT_SM, color: INK, opacity: .6, marginBottom: 7 }}>Phone</label>
              <input required type="tel" style={fieldStyle} />
            </div>

            <h2 style={{ fontFamily: DISPLAY, fontSize: TEXT_XL, color: HEADING_INK, margin: "0 0 18px", letterSpacing: "-.2px" }}>Shipping Address</h2>
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontFamily: SANS, fontSize: TEXT_SM, color: INK, opacity: .6, marginBottom: 7 }}>Address</label>
              <input required type="text" style={fieldStyle} />
            </div>
            <div className="featGrid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
              <div>
                <label style={{ display: "block", fontFamily: SANS, fontSize: TEXT_SM, color: INK, opacity: .6, marginBottom: 7 }}>City</label>
                <input required type="text" style={fieldStyle} />
              </div>
              <div>
                <label style={{ display: "block", fontFamily: SANS, fontSize: TEXT_SM, color: INK, opacity: .6, marginBottom: 7 }}>State</label>
                <input required type="text" style={fieldStyle} />
              </div>
            </div>
            <div style={{ maxWidth: 220, marginBottom: 40 }}>
              <label style={{ display: "block", fontFamily: SANS, fontSize: TEXT_SM, color: INK, opacity: .6, marginBottom: 7 }}>Pincode</label>
              <input required type="text" style={fieldStyle} />
            </div>

            <h2 style={{ fontFamily: DISPLAY, fontSize: TEXT_XL, color: HEADING_INK, margin: "0 0 18px", letterSpacing: "-.2px" }}>Payment</h2>
              ...
              {PAYMENT_METHODS.map(m => {
                ...
                return (
                  <button key={m} type="button" onClick={() => setPayment(m)}
                    style={{
                      fontFamily: SANS, fontSize: TEXT_BASE, borderRadius: 14, padding: "13px 20px", cursor: "pointer",
            ...
            <p style={{ fontFamily: SANS, fontSize: TEXT_XS, color: INK, opacity: .5, margin: 0 }}>Payments are securely processed via Razorpay.</p>
          </div>

          <div style={{ background: "#fff", borderRadius: 24, padding: 28, boxShadow: "0 4px 20px rgba(0,0,0,.05)" }}>
            <h2 style={{ fontFamily: DISPLAY, fontSize: TEXT_LG, color: HEADING_INK, margin: "0 0 18px", letterSpacing: "-.2px" }}>Order Summary</h2>
                ...
                return (
                  <div key={i.slug} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontFamily: DISPLAY, fontSize: TEXT_SM, color: INK }}>{p.name}</div>
                      <div style={{ fontFamily: SANS, fontSize: TEXT_XS, color: INK, opacity: .5 }}>Qty {i.qty}</div>
                    </div>
            ...
            <div style={{ borderTop: "1px solid rgba(23,23,23,.1)", paddingTop: 16, display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontFamily: SANS, fontSize: TEXT_BASE, color: INK, opacity: .68 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontFamily: DISPLAY, fontSize: TEXT_LG, color: INK, marginBottom: 20 }}>
              <span>Total</span><span>₹{total.toFixed(2)}</span>
            </div>

            <button type="submit" className="pbtn" style={{ width: "100%", background: INK, color: BG, border: "none", borderRadius: 100, padding: "17px 24px", fontFamily: DISPLAY, fontSize: TEXT_BASE, letterSpacing: "1px", textTransform: "uppercase", cursor: "pointer" }}>
              Pay ₹{total.toFixed(2)}
            </button>
          </div>
        </form>
