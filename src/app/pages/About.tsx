import { BG, PEACH, LAV, PINK, YELLOW, INK, HEADING_INK, DISPLAY, SCRIPT2, SANS } from "../theme";
  ...
  return (
    <div ref={fu.ref} style={fu.s}>
      <div style={{
        ...
        <h3 style={{ fontFamily: DISPLAY, fontSize: 20, color: HEADING_INK, margin: "0 0 12px", letterSpacing: "-.3px" }}>{v.title}</h3>
        <p style={{ fontFamily: SANS, fontSize: 14.5, color: INK, opacity: .72, lineHeight: 1.7, margin: 0 }}>{v.text}</p>
      </div>
    </div>
  );
}
    ...
    <>
      <section style={{ background: BG, paddingTop: 72 }}>
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "72px 24px 8px", textAlign: "center" }}>
          <div style={{ fontFamily: SCRIPT2, fontSize: 22, color: INK, opacity: .55, marginBottom: 6 }}>— our story —</div>
          <h1 style={{ fontFamily: DISPLAY, fontSize: "clamp(34px,5.5vw,64px)", color: HEADING_INK, margin: "0 0 20px", letterSpacing: "-1px", textTransform: "uppercase" }}>
            Sparkle, Made Responsibly.
          </h1>
          <p style={{ fontFamily: SANS, fontSize: 16, color: INK, opacity: .62, lineHeight: 1.8, margin: "0 auto", maxWidth: 640 }}>
      ...
      <section style={{ background: PEACH, padding: "88px 24px 0" }}>
        <div style={{ maxWidth: 720, margin: "0 auto", textAlign: "center" }}>
          <div style={{ fontFamily: SCRIPT2, fontSize: 22, color: INK, opacity: .5, marginBottom: 6 }}>— made in india —</div>
          <h2 style={{ fontFamily: DISPLAY, fontSize: "clamp(28px,3.6vw,46px)", color: HEADING_INK, margin: "0 0 18px", letterSpacing: "-.5px", textTransform: "uppercase", lineHeight: 1.15 }}>
            Crafted Close to Home, Shipped Across the Country
          </h2>
          <p style={{ fontFamily: SANS, fontSize: 16, color: INK, opacity: .68, lineHeight: 1.8, margin: 0 }}>
