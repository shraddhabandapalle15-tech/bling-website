...
const fieldStyle = {
  ...
  fontFamily: SANS, fontSize: 15, color: INK, background: "#fff", outline: "none", boxSizing: "border-box" as const,
};
  ...
  return (
    <section style={{ background: BG, paddingTop: 72, minHeight: "100vh" }}>
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "72px 24px 100px" }}>
        <div style={{ fontFamily: SCRIPT2, fontSize: 22, color: INK, opacity: .55, marginBottom: 6 }}>— get in touch —</div>
        <h1 style={{ fontFamily: SANS, fontWeight: 700, fontSize: "clamp(34px,5vw,56px)", color: HEADING_INK, margin: "0 0 44px", letterSpacing: "-0.5px" }}>
          Contact Us
        </h1>
        ...
        <div className="shopLayout" style={{ display: "grid", gridTemplateColumns: "1.3fr 1fr", gap: 48, alignItems: "start" }}>
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 18 }}>
            <div>
              <label style={{ display: "block", fontFamily: SANS, fontSize: 13, color: INK, opacity: .6, marginBottom: 7 }}>Name</label>
            ...
            <div>
              <label style={{ display: "block", fontFamily: SANS, fontSize: 13, color: INK, opacity: .6, marginBottom: 7 }}>Message</label>
              <textarea required rows={5} style={{ ...fieldStyle, resize: "vertical" as const }} />
            </div>
            <button type="submit" className="pbtn" style={{ background: INK, color: BG, border: "none", borderRadius: 100, padding: "16px 32px", fontFamily: DISPLAY, fontSize: 14, letterSpacing: "1px", textTransform: "uppercase", cursor: "pointer" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {CHANNELS.map(c => (
              <div key={c.label} style={{ border: "1.5px solid rgba(23,23,23,.12)", borderRadius: 18, padding: "18px 20px", display: "flex", alignItems: "center", gap: 14, background: "#fff" }}>
                ...
                <div>
                  <div style={{ fontFamily: DISPLAY, fontSize: 11, letterSpacing: "1px", textTransform: "uppercase", color: INK, opacity: .5, marginBottom: 3 }}>{c.label}</div>
                  <div style={{ fontFamily: SANS, fontWeight: 700, fontSize: 15.5, color: INK }}>{c.value}</div>
                </div>
              </div>
            ))}
          </div>
