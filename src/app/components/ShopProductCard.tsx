...
export function ShopProductCard({ p, delay }: { p: Product; delay: number }) {
  ...
  return (
    <div ref={fu.ref} style={fu.s}>
      <Link to={`/product/${p.slug}`} style={{ textDecoration: "none", color: "inherit", display: "flex", height: "100%" }}>
        <div
          style={{
            background: "#fff", borderRadius: 28, overflow: "hidden", height: "100%", width: "100%",
        ...
        >
          <div style={{ position: "relative", background: p.bg, width: "100%", aspectRatio: "4 / 3", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
            {badgeLabel && (
              <div style={{ position: "absolute", top: 14, left: 14, zIndex: 1, ...badgeStyle, borderRadius: 100, padding: "5px 13px", fontSize: TEXT_2XS, letterSpacing: ".5px", textTransform: "uppercase", fontFamily: DISPLAY }}>
                {badgeLabel}
              </div>
            )}
